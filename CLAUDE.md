...
MeetSpot is an **AI Agent** for multi-person meeting point recommendations. Unlike passive search tools, it makes autonomous decisions: users provide locations and requirements, the Agent calculates the geographic center and recommends optimal venues. Built with FastAPI and Python 3.11+, it uses the Amap (Gaode Map) API for geocoding and POI search.
...
```
User Request → Complexity Router → Rule Mode (simple) or Agent Mode (complex)
                                          ↓
...
| Mode | Trigger | Speed |
|------|---------|-------|
| Rule Mode | < 40 complexity score | 2-4s |
| Agent Mode | ≥ 40 complexity score | 8-15s |

Complexity scoring: +10/location, +15 for complex keywords, +10 for special requirements.
...
|------|---------|---------------|
| Full | `config/config.toml` exists | TOML via `app/config.py` |
